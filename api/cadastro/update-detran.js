...
import { validate, updateDetranSchema } from '../../lib/validators.js';
import db from '../../lib/db.js';

async function handler(req, res) {
  const validation = validate(updateDetranSchema, req.body);
  if (!validation.success) {
    return res.status(400).json({ error: 'Dados invalidos', details: validation.errors });
  }

  const { documentId, password, credenciamentoDetran } = validation.data;
  ...
  try {
    ...
    dadosCadastro.credenciamentoDetran = credenciamentoDetran;
    ...
    console.log('DETRAN updated for user:', userId);
    return res.status(200).json({ success: true, message: 'Credenciamento DETRAN atualizado com sucesso' });
  } catch (err) {
    console.error('DETRAN update failed for user:', userId);
    return res.status(500).json({ error: 'Falha ao atualizar credenciamento DETRAN' });
  } finally {
